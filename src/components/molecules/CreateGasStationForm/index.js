...
const CreateGasStationForm = () => {
  ...
  const {
    ...
    onSubmit: (values) => {
      ...
      dispatch(GasStationActions.createGasStationRequest(filtredValues))
    },
  })
  const commomEvents = { onChange: handleChange, onBlur: handleBlur, onFocus: handleBlur }
  const isSendingRequest = useSelector(store => store.gasStation.isFetching)
  ...
  const { cities, states } = useSelector(store => ({ states: store.state.states, cities: store.city.cities }))
...
export default CreateGasStationForm
