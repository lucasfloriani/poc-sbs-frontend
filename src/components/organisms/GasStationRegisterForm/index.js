...
const GasStationRegisterForm = ({ history, isFetching, publicCreateGasStationRequest }) => (
  <Formik
    ...
    onSubmit={(values) => {
      ...
      publicCreateGasStationRequest(filtredValues, () => history.replace({ pathname: '/' }))
    }}
    validationSchema={
      Yup.object().shape({
        email: Yup.string()
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(GasStationRegisterForm))
